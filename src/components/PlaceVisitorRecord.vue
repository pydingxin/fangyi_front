...
export default {
  ...
  methods: {
    queryInfo(){
      ...
      let that=this;
      let jdata={name:that.name, ldt:that.ldt, rdt:that.rdt}
      this.waiting_result=true;
      this.post_api('place_visitor_record', jdata, (ret)=>{
...
<template>
  <p class="txt">场所人员扫码记录【若无记录，可联系指挥部丁鑫或李建光，到场所码系统尝试查询】 </p>
  <a-space>
    <a-input v-model:value="name"  id="input" placeholder="输入场所名" />
    ...
    <a-button id="outportBtn" @click="JSONToExcelConvertor(data, '场所扫码人员')" size="large" type="primary">导出excel</a-button>
